...
read.logic = async (req) => {
    ...
    const [count, requests] = await Promise.all([
        database.client.collection(collections.CATALOGES).countDocuments(key),
        database.client.collection(collections.CATALOGES).find(key).skip(offset).limit(limit).toArray()
    ]);
